...
class Network:
    ...
    def __init__(self, layers,final_centroids,final_clusters):
    ...
    def stochastic_gradient_descent(self, training_set, learning_rate, epochs):
        ...
        for i in range(epochs):
            ...
            for tuple_ in training_set:
                self.update_weights(tuple_, len(training_set), learning_rate)

            print("Training Accuracy in Epoch", i+1, ": ", end="")
            print('%.3f%%' % ((self.network_accuracy(training_set)/len(training_set))*100))

    def update_weights(self, tuple_, training_set_size, learning_rate):
        ...
        for k in range(len(self.weights)):
            self.weights[k] -= learning_rate * weight_gradients[k]
    ...
    def backpropagation(self, image, label):
        ...
        # Feed Forward
        activations = []
        transfers = []  #try with image here

        hidden_output = np.reshape(np.asarray([[self.gaussian(image, mu, cluster)] for mu, cluster in zip(self.centroids, self.clusters)]), (len(self.centroids), 1))
        network_output = np.dot(self.weights[0], hidden_output) + self.biases[0]

        activations.append(image)
        transfers.append(hidden_output)
        transfers.append(network_output)
    ...
    def network_accuracy(self, testing_set):
        results = [(np.argmax(self.feed_forward(image)), np.argmax(label)) for (image, label) in testing_set]
...
def k_fold_cross_validation(dataset, num_folds, learning_rate, epochs, final_centroids, final_clusters):
    ...
    for i in range(num_folds):
        ...
        network = Network(layers,final_centroids,final_clusters)
        network.stochastic_gradient_descent(training_folds, learning_rate, epochs)
